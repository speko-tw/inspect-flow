...
- **Issue 與 PR 欄位**（設定方式與 id 見 [`docs/specs/README.md`](docs/specs/README.md#issue-pr-fields)）：
  - Issue：assignee（目前是 `kavy7884`）、Issue Type（`Task` 或 `Bug`，開完確認有套用）、area label、Milestone（依 [`docs/intents/06-versioning-and-milestone-governance.md`](docs/intents/06-versioning-and-milestone-governance.md#issue-ownership-table) 判斷所屬版本系列），以及組織欄位 Priority、Effort，一律填寫。
  - PR：assignee、area label、Milestone 與它關閉的 issue 相同（見 [`docs/intents/06-versioning-and-milestone-governance.md`](docs/intents/06-versioning-and-milestone-governance.md#pr-ownership-table)）；PR 沒有 Priority、Effort。
