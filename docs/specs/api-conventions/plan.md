...
| T3 | multipart 慣例契約測試：測試模組內建立一個臨時 `APIRouter`，掛一個只接受 `UploadFile` 的路由；分別以 multipart 與 JSON base64 送出同一檔案。JSON base64 因缺少必要的 multipart 欄位，由 FastAPI 產生 `RequestValidationError`，經 T1 的 handler 轉成統一 envelope（4xx），不需要另外實作特定狀態碼的處理邏輯 | `backend/tests/contract/test_multipart_conventions.py`（新增）、`backend/pyproject.toml`（新增 `python-multipart` 依賴）、`backend/uv.lock` | T1（統一錯誤 envelope 與 handler） | API-AC06 | #? |
...
| API-AC11 | `backend/tests/contract/test_time_format.py`：呼叫測試路由，以 `datetime.fromisoformat`（或等效 RFC 3339 解析器）解析回應時間欄位字串，斷言解析成功、`tzinfo` 為 UTC（offset 0）、`microsecond == 0`、字串以 `Z` 結尾；並對一個無效範例（例如 `2026-99-99T99:99:99Z`）斷言解析失敗或未通過上述斷言 |
