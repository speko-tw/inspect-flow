...
2. **規格內標示**：標頭「狀態」寫「部分凍結」，並在「被擋議題」下加一行 `**凍結範圍**：`，列出已凍結的實體與涵蓋它們的需求、驗收條件編號；沒列到的都是草稿。計畫與任務只能涵蓋凍結範圍內的部分。範本標頭「裁定前本規格只能維持草稿」的提示，在部分凍結時只適用於凍結範圍以外的部分。擴大凍結範圍時，在同一份規格的 PR 裡更新這一行與比對結果；全部凍結後改為「已凍結」並刪掉這一行。理由：沿用範本既有的標頭，不必改範本，讀規格的人一眼就知道哪些能拆任務。
