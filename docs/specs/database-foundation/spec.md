...
`User`、`Project` 的命中都只是「其他實體引用它的 UUID」，依規則 1 的「只用 ID 引用」例外判為無關（見 [DBF-Q3](#dbf-q3)）。
...
<a id="dbf-q3"></a>
- **DBF-Q3：部分凍結規則 1 要不要補「只引用 ID 不算有關」的例外**（已裁定，[#55](https://github.com/speko-tw/inspect-flow/issues/55)）。本規格把 `User`、`Project` 的字面命中判為無關（見[資料](#資料)段），但規則 1 原文是「字面可能指到它，就算有關」。寫 `domain-model` 時會再碰到同樣的判斷，要不要改規則由負責人決定。
