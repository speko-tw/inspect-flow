...
- 每個任務至少對應一條 AC；DOM-AC01～DOM-AC18 每條都被一個任務涵蓋。
...
- **裁定未完成就開工**：T1～T3 的欄位長度、唯一約束與預設值依 DOM-Q1～DOM-Q7。任一裁定未完成時，對應任務標 `blocked`；不要先用猜的長度或預設值建表，事後改約束要多一支 migration。
