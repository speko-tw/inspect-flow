...
- 密碼與其雜湊、登入流程、session 或 token、「目前使用者」的辨識、API 權限檢查的執行方式（含後端預設拒絕、Admin 可存取所有專案、誰可以建立帳號與指派角色）：移至 `authentication`，受 [OQ-13](../../intents/05-open-questions.md#oq-13) 擋。本規格只定義資料與規則，`authentication` 依此執行。
