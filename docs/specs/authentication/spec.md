...
<a id="aut-q2"></a>
- **AUT-Q2：Admin 是否也通過特殊動作（例如簽核、匯出列印）的權限檢查**（AUT-R19）。[KD-24](../../intents/03-decisions-and-stack.md#kd-24) 寫「查看、修改所有專案」，沒提特殊動作。選項：（A）Admin 通過所有專案權限代碼，含特殊動作；（B）Admin 只通過讀取、新增、修改、刪除，特殊動作仍要專案角色。**建議 A**：Admin 本來就能把自己加進專案並指派任何角色，B 只多一道手續，擋不住 Admin；若簽核要求「本人親自具備角色」，應在該功能規格另訂。影響計畫 T4。
<a id="aut-q3"></a>
