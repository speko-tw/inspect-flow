...
| T6 | 設定密碼的指令：以 email 指定帳號，`getpass` 輸入兩次，也接受標準輸入；長度規則檢查；拒絕不存在與 `external` 帳號；寫入或更新 `UserPassword`（操作者為內建 `admin`）並刪除該帳號所有 `AuthSession`；指令列不提供密碼參數；在 `Makefile` 加一個執行入口 | `backend/app/cli/set_password.py`（新增；`backend/app/cli/` 套件由 DOM T6 建立）、`backend/app/auth/passwords.py`（加長度規則）、`Makefile`（加一個 target）、`backend/tests/cli/test_set_password.py`（新增） | T1、T2、T3（刪除登入狀態）、#134；[AUT-Q3](spec.md#aut-q3)（長度規則）、[AUT-Q4](spec.md#aut-q4)（內建 `admin` 能否設定密碼）裁定 | AUT-AC04、AUT-AC23～AUT-AC25 | #154 |
...
- **裁定未完成就開工**：T3 的裁定都已完成（AUT-Q1、DOM-Q2）；T4 依 AUT-Q2、DOM-Q3；T6 依 AUT-Q3、AUT-Q4；T8 依 AUT-Q5。不要先用建議值實作再等裁定；建議值寫在規格裡，是給負責人選的，不是預設。
