...
**這份文件回答**：某個中文說法或英文欄位，對應到系統裡的哪一個實體？它跟其他實體是什麼關係？
**什麼時候讀**：命名新欄位、討論資料模型，或看到「work order」這類非正式用語時，先查這裡再自創新名詞。

領域名詞、對應的英文系統實體名、定義，以及它與其他名詞的關聯。凡是原文以中文表達的概念，第一次出現 時保留其來源用語；系統實體名稱一律使用英文（例如 `Inspection Task`、`Evidence`），符合原始碼與資料庫 中的實際命名。
...
| 公司 | `Company` | 人員所屬的組織，含本公司體系（`kind = internal`）與客戶（`kind = customer`）；`code` 不能重複，`tax_id`（統一編號）選填但填了不能重複，`parent_id` 指向母公司（依據：負責人決定（#63，2026-09-26）；架構基準無對應章節，見 [KD-23](03-decisions-and-stack.md#kd-23)）。 | 一個 `User` 屬於一個 `Company`，可隨時修改；有獨立營業登記的分公司、子公司都算獨立的 `Company`。 |
| 系統管理者 | `Admin`（`User.is_admin`） | 人員身上的一個開關，不是角色：管理系統設定、人員、公司、角色定義，並可直接查看、修改所有專案（依據：負責人決定（#63，2026-09-26）；取代架構基準 §17 的範例矩陣，見 [KD-24](03-decisions-and-stack.md#kd-24)）。 | 與「專案角色」是兩套不同機制，不透過 `Role`／`ProjectMember` 授予；不能拿掉最後一個 Admin，見 [KD-29](03-decisions-and-stack.md#kd-29)。 |
| 專案角色 | `Role` | 全系統共用一份清單、可自訂的角色；首次安裝預建三個範本角色（內業整理、現場查核、唯讀），之後可改名、改內容或刪除，修改立即影響所有持有者（依據：負責人決定（#63，2026-09-26）；取代架構基準 §17 的範例矩陣，見 [KD-26](03-decisions-and-stack.md#kd-26)）。 | 掛在 `ProjectMember` 上；各範本角色勾選哪些權限在系統畫面上調整，不寫進本文件，見 [OQ-08](05-open-questions.md#oq-08)。 |
| 權限 | 權限代碼，例如 `report.read`、`report.approve` | 「資料 × 動作」的組合：讀取、新增、修改、刪除，再加上簽核、匯出列印等特殊動作（依據：負責人決定（#63，2026-09-26）；架構基準無對應章節，見 [KD-25](03-decisions-and-stack.md#kd-25)）。 | 是「專案角色」的組成內容；系統存的是權限代碼，不是寫死的功能表。 |
...
- 這些名詞在什麼規則下必須／應／得如何處理，見 [02-principles.md](02-principles.md)；決策脈絡見
  [03-decisions-and-stack.md](03-decisions-and-stack.md)。
- 名詞背後仍未拍板或來源自相矛盾的部分，見 [05-open-questions.md](05-open-questions.md)。
- 系統整體生命週期與各名詞在流程中的位置，見 [01-overview.md](01-overview.md)。
