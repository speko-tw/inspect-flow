...
| 證據變體 — 原圖 | `Evidence Variant`（`variant_type = ORIGINAL`） | 使用者提交當下的原始檔案，永不覆蓋、永不修改（依據：架構基準 §13A.2、§13A.4）。 | 是所有其他 Variant（`EDITED`／`THUMBNAIL`／`REPORT`）回溯的終點；與 `Evidence` 的關係在來源文件中不完全一致，見 [06-open-questions.md](06-open-questions.md) 的「來源內部不一致」G-02。 |
...
| 儲存鍵 | `storage_key` | 後端以 Evidence／檔案的 UUID 產生的檔案儲存位置識別碼，例如 `evidence/<uuid>.jpg`（依據：架構基準 §13.1–13.2）。 | 只是「檔案在哪」的內部定位符，**不是**授權憑證，也不是原始檔名或業務編號；不同章節給的儲存鍵範例路徑並不一致，見 [06-open-questions.md](06-open-questions.md) G-02。 |
